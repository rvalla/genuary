...
def gen_29c(sq_size, mg, grid_size, background, text, name):
	"The function to create a beautiful grid based on text..."
	...
	for r in range(grid_size):
		for c in range(grid_size):
				canvas.draw_line(ut.get_color_tuple(row_matrix[r][c]), l_w, (r*sq_size[1]+mg[1],c*sq_size[0]+mg[0]), (r*sq_size[1]+mg[1], (c+1)*sq_size[0]+mg[0]))
				canvas.draw_line(ut.get_color_tuple(col_matrix[r][c]), l_w, (r*sq_size[1]+mg[1], c*sq_size[0]+mg[0]), ((r+1)*sq_size[1]+mg[1], c*sq_size[0]+mg[0]))
