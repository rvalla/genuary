...
class Gen13():
	...
	def hexagram(self, colors, color_motion, size_factor, constant_size):
		...
		for s in strips:
			...
			if s < 2:
				if s == 0:
					ut.gen_alpha_rectangle(self.canvas, self.hw[1], height, location, self.densities[0], [color], color_motion[0], factors, size_factor, constant_size)
				else:
					ut.gen_alpha_rectangle(self.canvas, self.hw[1], height, location, self.densities[0], [color], color_motion[1], factors, size_factor, constant_size)
			else:
				if s == 2:
					ut.gen_alpha_rectangle(self.canvas, 2 * space, height, location, self.densities[1], [color], color_motion[0], factors, size_factor, constant_size)
					location[1] += space * 3
					ut.gen_alpha_rectangle(self.canvas, 2 * space, height, location, self.densities[1], [color], color_motion[0], factors, size_factor, constant_size)
				else:
					ut.gen_alpha_rectangle(self.canvas, 2 * space, height, location, self.densities[1], [color], color_motion[1], factors, size_factor, constant_size)
					location[1] += space * 3
					ut.gen_alpha_rectangle(self.canvas, 2 * space, height, location, self.densities[1], [color], color_motion[1], factors, size_factor, constant_size)
			y += 2 * height
